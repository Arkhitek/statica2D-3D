document.addEventListener('DOMContentLoaded', () => {
    ...
    const nodesData = initialNodes.map(n => {
        // Normalize boundary condition for display
    ...
    const membersData = initialMembers.map(m => {
        ...
        // Handle 'Pinned' as 'Pin' for display
    ...
    function initNodesSheet() {
        ...
        console.log('Initializing nodes sheet');
        try {
            ...
            console.log('Nodes sheet initialized successfully');
        } catch (e) {
    ...
    function initMembersSheet() {
        ...
        console.log('Initializing members sheet');
        try {
            ...
            console.log('Members sheet initialized successfully');
        } catch (e) {
    ...
    function initNodeLoadsSheet() {
        ...
        console.log('Initializing node loads sheet');
        try {
            ...
            console.log('Node loads sheet initialized successfully');
        } catch (e) {
    ...
    function initMemberLoadsSheet() {
        ...
        console.log('Initializing member loads sheet');
        try {
            ...
            console.log('Member loads sheet initialized successfully');
        } catch (e) {
    ...
    // --- Initial Load ---
    console.log('Loading initial tab (nodes)');
    initNodesSheet();
    ...
    document.getElementById('reflect-btn').addEventListener('click', () => {
        ...
        try {
            // Initialize any uninitialized sheets
            if (!nodesSheet) { 
                nodesContainer.style.display = 'block'; 
                initNodesSheet(); 
                nodesContainer.style.display = 'none'; 
            ...
            console.log('Retrieved data:', { nodes: nData.length, members: mData.length, nodeLoads: nlData.length, memberLoads: mlData.length });
            ...
            nData.forEach(row => {
                ...
                // Normalize boundary condition from display to internal format
                ...
                nodes.push({
                    x: x, y: y, fix: fixValue, dx: row[3], dy: row[4], rot: row[5]
                });
            });
            ...
            nlData.forEach(row => {
                ...
                nodeLoads.push({
                    node: node, px: row[1], py: row[2], mz: row[3]
                });
            });
            ...
            mlData.forEach(row => {
                ...
                memberLoads.push({
                    member: member, w: row[1]
                });
            });

            console.log('Parsed data:', { nodes: nodes.length, members: members.length, nodeLoads: nodeLoads.length, memberLoads: memberLoads.length });
            ...
            if (window.opener && window.opener.updateFromSpreadsheet) {
                console.log('Calling updateFromSpreadsheet');
